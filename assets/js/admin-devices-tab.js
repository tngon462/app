// assets/js/admin-devices-tab.js (clean)
// - Widget: chỉ 1 công tắc blackout theo BÀN, click vào CARD mở popup thao tác
// - Toolbar: Reload toàn bộ + Bật/Tắt toàn bộ (toggle one-button)
// - Sắp xếp theo số bàn để UI không nhảy
// - Chịu được admin.html khác nhau (tbody / grid)
...
(function(){
  ...
  async function loadLinks(){
    try{
      ...
      LINKS_MAP=null; warn('Không tải được links.json');
    }
  }
  ...
  function ensureToolbar(db){
    ...
    $('#btnToggleAll', bar).addEventListener('click', async ()=>{
      ...
      try{
        ...
        // Re-render ngay để đổi màu widget
        if (devicesGrid) renderGrid(devicesGrid, db, devicesCache);
      }catch(e){ alert('Ghi trạng thái toàn bộ lỗi: '+(e?.message||e)); }
    });
  }

  // ===== Containers =====
  let devicesGrid = null;
  function pickContainers(){
    const tbody = document.getElementById('devBody') || document.getElementById('devices-tbody') || document.getElementById('devices_tbody');
  ...
  // ===== Render bảng (tuỳ admin muốn dùng dạng table) =====
  ...
  // ===== Render grid widget (click card mở popup; 1 toggle blackout) =====
  ...
  document.addEventListener('DOMContentLoaded', async ()=>{
    try{
      ...
      db.ref('control/tables').on('value', s=>{
        const v = s.val()||{};
        Object.keys(v).forEach(k=> tableScreen[String(k)] = (v[k]?.screen || v[k])); // 'on'/'off' hoặc {screen:'on'}
        if (grid) renderGrid(grid, db, devicesCache);
